...
static int init_detect_gpios(void)
{
	int ret;
	int u2_det = 1;
	int detect = 0;
	int value = 0;
	int delay_time = env_get_ulong("hdd-spinup-mdelay", 10, 2000);
	ofnode node;
	struct gpio_desc desc;
	printf("hdd-spinup-mdelay : %d\n", delay_time);

	ofnode_for_each_compatible_node(node, "ai-nas,gpio-detect") {
		if (!ofnode_valid(node)) {
			printf("%s: Error parsing FDT node %s\n",
			      __func__, ofnode_get_name(node));
			break;
		}

		ret = gpio_request_by_name_nodev(node, "detect1-gpios", 0, &desc, GPIOD_IS_IN);
		if (ret == 0)
		{
			u2_det = dm_gpio_get_value(&desc);
		}

		ret = gpio_request_by_name_nodev(node, "detect-gpios", 0, &desc, GPIOD_IS_IN);
		if (ret != 0)
		{
			printf("Init %s failed, ret=%d\n", node.np->name, ret);
		}
		detect = dm_gpio_get_value(&desc);

		value = (u2_det && detect) ? 0 : 1;

		ret = dm_gpio_set_dir_flags(&desc, GPIOD_IS_OUT);
		if (ret) {
			printf("Failed to set output direction, ret=%d\n", ret);
			return ret;
		}
